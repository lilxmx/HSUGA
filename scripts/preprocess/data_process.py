...
def Amazon(dataset_name, rating_score):
    ...
    for inter in parse(data_flie):
        if float(inter['overall']) <= rating_score:  # Filter out ratings below threshold
            continue
...
def New_Amazon(dataset_name, rating_score):
    ...
    datas = []
    # Use relative path, relative to project root directory
    data_flie = "./data/beauty/raw/2023/Amazon_Fashion.jsonl.gz"
    # Alternative: data_flie = './data/' + str(dataset_name) + '/raw/' + str(dataset_name) + '.json.gz'
    for inter in parse(data_flie):
        if float(inter['rating']) <= rating_score:  # Filter out ratings below threshold
            continue
        user = inter['user_id']
        item = inter['asin']
...
def Yelp(date_min, date_max, rating_score): # take out inters in [date_min, date_max] and the score < rating_score
    ...
    for line in tqdm(lines):
        ...
        rating = review['stars']
        # Date format: 2004-10-12 10:13:32 to 2019-12-13 15:51:19
        date = review['date']
        # Filter out examples outside date range or below rating threshold
...
# Categories and brand are all attributes
...
def filter_common(user_items, user_t, item_t):
    ...
    for user, item, timestamp in user_items:
        # Filter the original dataset: remove users and items with interactions less than threshold.
        # Note: After filtering, recalculated counts may not satisfy the requirement.
        if user_count[user] < user_t or item_count[item] < item_t:
            continue
        if user not in User.keys():
            User[user] = []
        User[user].append((item, timestamp))
    # User dict has been filtered. Some users may have fewer than 3 interactions because
    # items with less than 3 interactions were removed from their sequences.
    new_User = {}
...
def main(data_name, data_type='Amazon', user_core=3, item_core=3, rating_score=0.0, is_saved=False):
    assert data_type in {'Amazon', 'Yelp', 'New_Amazon'}
    np.random.seed(12345)
    # rating_score = 3  # rating score smaller than this score would be deleted
    ...
    user_items, user_num, item_num, data_maps = id_map(user_items)  # Map to new numeric IDs
    user_items = filter_minmum(user_items, min_len=3)  # Filter users with interactions less than min_len
...
def LastFM():
    ...
    item_core = 5
    datas = []
    # Note: Update this path to your LastFM dataset location
    data_file = './data/lastfm/2k/user_attributegedartists-timestamps.dat'
    ...
    # Note: Update this path to your LastFM attribute file location
    attribute_file = './data/lastfm/artist2attributes.json'
...
if __name__ == "__main__":
    ...
    # main("fashion", data_type="New_Amazon", user_core=5, item_core=5, is_saved=False)
    main("beauty", data_type="Amazon", user_core=5, item_core=5, is_saved=False)